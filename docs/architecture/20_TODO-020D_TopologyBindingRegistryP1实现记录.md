# TODO-020D TopologyBindingRegistry P1 Partial Hardening 实现记录

## 结论

TODO-020D 完成了 `TopologyBindingRegistry` 的 P1 partial hardening：

```text
edge endpoint canonicalization
restore fallback semantics
fallback ambiguity diagnostics
localIndex+bbox low-confidence guard
realistic STEP topology stress tests
```
...
geometryFingerprint 也漂移时，fallbackLocalIndex + fallbackBbox 只作为 last-resort
诊断线索，返回 BINDING_LOW_CONFIDENCE，不自动恢复成功。
...
TopologyBinding 尚未达到 production-ready graph binding。
Viewer 选择系统已经接入。
...
4. 若 geometryFingerprint 多候选，直接返回 BINDING_AMBIGUOUS。
5. 若 geometryFingerprint 无候选，fallbackLocalIndex + fallbackBbox 只返回
   BINDING_LOW_CONFIDENCE，不自动 ok。
6. 多候选 -> BINDING_AMBIGUOUS。
7. 无候选 -> BINDING_MISSING。
```

fallback 只在唯一候选时返回成功。
其中唯一候选必须来自 geometryFingerprint 匹配。
纯 localIndex + bbox 命中不再算成功，避免真实 STEP re-import 后静默误绑。
...
### 4. 外审后 low-confidence 收口

Web GPT Pro 外审指出：

```text
localIndex 在 STEP re-import 中不是稳定保证。
localIndex + bbox fallback 自动成功会埋 silent wrong binding。
```

因此追加调整为：

```text
geometryFingerprint fallback:
  唯一候选 -> ok + usedFallback
  多候选 -> BINDING_AMBIGUOUS

localIndex + bbox fallback:
  唯一候选 -> BINDING_LOW_CONFIDENCE + usedFallback + candidateCount=1
  多候选 -> BINDING_AMBIGUOUS
```

上层在 TODO-020E / TODO-021 中遇到 `BINDING_LOW_CONFIDENCE` 时，
只能提示用户重新选择或进入人工确认流程，不能静默替换绑定。
...
5. stableId / geometryFingerprint 都漂移但 localIndex + bbox 匹配时返回
   BINDING_LOW_CONFIDENCE，不自动恢复。
6. fallback 全零 bbox 也返回 BINDING_LOW_CONFIDENCE。
...
12. realistic compound STEP 连续导入 / restore determinism 稳定。
13. realistic fused L-shape STEP 连续导入 / restore determinism 稳定。
```
...
TODO020B-GAP-008:
  ...
  -> TODO-020D 已通过 geometryFingerprint fallback 关闭。
     localIndex / bbox 仅作为 low-confidence 诊断线索，不作为自动恢复依据。
...
| TODO020D-GAP-005 | 当前仍是 heuristic matcher，不是 deterministic topology graph system。 | 后续 graph hash / adjacency context /真实 STEP collision 样本驱动扩展。 |
| TODO020D-GAP-006 | realistic compound / fused L-shape 测试是程序生成的 realistic stress，不是人工业务 golden STEP。 | 后续补真实业务 STEP GC-STEP-TOPO 样本。 |
